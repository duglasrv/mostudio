...
| `pricing.html` | 3 paquetes (Instante / Momento / Legado) + bloque de notas (extras, movilización, forma de pago, cotización para bodas/XV años) |
...
## Deploy
- **Objetivo**: Vercel, importando `https://github.com/duglasrv/mostudio`
  directo (framework preset **Other** — es HTML estático, no hay build
  command ni output directory especial).
- **Dominio**: se decidió usar un subdominio del `.dev` que Omar ya tiene
  para su portafolio de developer (`rvstudio.dev`), en vez de comprar un
  dominio nuevo o reusar la raíz — propuesta: `foto.rvstudio.dev`.
  Pendiente: confirmar si el DNS de `rvstudio.dev` vive en Namecheap o ya
  fue delegado a Vercel, para saber dónde agregar el registro CNAME que
  Vercel pida al conectar el subdominio.
...
mantenerse** en todas las páginas por requisito de licencia.
